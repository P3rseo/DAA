...
def es_factible(sol, row, col):
    ...
    while factible and i <= row:
        factible_col = (sol[row - i] != col)
        factible_diag_45 = (sol[row -i] != col - i) # Fila superior a la izquierda
        factible_diag_135 = (sol[row - i] != col + i) # Fila superior a la derecha
        factible = factible_col and factible_diag_45 and factible_diag_135
        i += 1
