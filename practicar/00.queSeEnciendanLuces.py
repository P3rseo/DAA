...
def maximizar_tentacion(concursantes):
    candidatosFinales = []
    for concursante in concursantes:
        ...
        cualidad, tiempoMax, candidatos = concursante
        if cualidad == "beauty":
        ...
        for nombre, b, i, k, t in candidatos:
            ...
            if tiempoActual + t <= tiempoMax:
                ...
                if resto > 0:
                    fraccion = seleccionCualidad / t
                    tiempoActual += t * fraccion
                    tentacionTotal += seleccionCualidad * fraccion
                    nombres.append(nombre)
                    break
        candidatosFinales.append([nombres, tentacionTotal])
...
tentadores = maximizar_tentacion(concursantes)
for nombres, tentacionTotal in tentadores:
    print(" ".join(nombres))
